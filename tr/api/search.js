...
export default async function handler(req, res) {
    ...
    for (let i = 0; i < keys.length; i++) {
        ...
        try {
            // إضافة &lr=lang_tr&hl=tr للحصول على نتائج باللغة التركية
            const apiUrl = `https://www.googleapis.com/customsearch/v1?key=${currentKey}&cx=${CX}&q=${encodeURIComponent(q)}&num=10&start=${start}&lr=lang_tr&hl=tr`;
            ...
            if (response.ok) {
                ...
                console.log(`✅ نجاح: تم استخدام المفتاح رقم ${i + 1} (اللغة: التركية)`);
