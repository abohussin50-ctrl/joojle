...
export default async function handler(req, res) {
    ...
    for (let i = 0; i < keys.length; i++) {
        ...
        try {
            // تم إضافة &lr=lang_ru&hl=ru لجلب النتائج باللغة الروسية
            const apiUrl = `https://www.googleapis.com/customsearch/v1?key=${currentKey}&cx=${CX}&q=${encodeURIComponent(q)}&num=10&start=${start}&lr=lang_ru&hl=ru`;
            ...
            if (response.ok) {
                ...
                console.log(`✅ نجاح: تم استخدام المفتاح رقم ${i + 1} (اللغة: الروسية)`);
            ...
            if (response.status === 429 || response.status === 403) {
                console.warn(`⚠️ تنبيه: المفتاح رقم ${i + 1} استهلك حصته.`);
                continue; 
            } else {
