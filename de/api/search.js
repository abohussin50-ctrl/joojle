...
export default async function handler(req, res) {
    ...
    for (let i = 0; i < keys.length; i++) {
        ...
        try {
            // إضافة lr=lang_de و hl=de لضمان ظهور النتائج باللغة الألمانية
            const apiUrl = `https://www.googleapis.com/customsearch/v1?key=${currentKey}&cx=${CX}&q=${encodeURIComponent(q)}&num=10&start=${start}&lr=lang_de&hl=de`;
            ...
            if (response.ok) {
                ...
                console.log(`✅ نجاح: تم استخدام المفتاح رقم ${i + 1} (اللغة الألمانية)`);
