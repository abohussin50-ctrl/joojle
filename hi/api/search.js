...
export default async function handler(req, res) {
    ...
    for (let i = 0; i < keys.length; i++) {
        ...
        try {
            // تم إضافة &lr=lang_hi&hl=hi للحصول على نتائج باللغة الهندية
            const apiUrl = `https://www.googleapis.com/customsearch/v1?key=${currentKey}&cx=${CX}&q=${encodeURIComponent(q)}&num=10&start=${start}&lr=lang_hi&hl=hi`;
            ...
            if (response.ok) {
                ...
                console.log(`✅ نجاح: تم استخدام المفتاح رقم ${i + 1} (اللغة: الهندية)`);
