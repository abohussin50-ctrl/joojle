...
export default async function handler(req, res) {
    ...
    for (let i = 0; i < keys.length; i++) {
        ...
        try {
            // تم إضافة &lr=lang_it&hl=it للحصول على نتائج باللغة الإيطالية
            const apiUrl = `https://www.googleapis.com/customsearch/v1?key=${currentKey}&cx=${CX}&q=${encodeURIComponent(q)}&num=10&start=${start}&lr=lang_it&hl=it`;
            ...
            if (response.ok) {
                ...
                console.log(`✅ نجاح: تم استخدام المفتاح رقم ${i + 1} (اللغة: الإيطالية)`);
